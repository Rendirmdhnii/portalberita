...
export default function Home({
  ...
  const fetchData = async () => {
    try {
      ...
      const finalBeritaTerbaru = beritaTerbaru
        .filter((berita) => !usedIds.includes(berita.id))
        .slice(0, 15);

      setCategories(catData || []);
      setBerita(finalBeritaTerbaru);
              ...
              {sorotanNews.map((berita) => (
                <div key={berita.id} className="flex-none w-[75vw] sm:w-[260px] md:w-[300px] bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="p-3 md:p-4">
                     ...
                     <h3 className="text-sm md:text-base font-bold text-gray-900 leading-snug line-clamp-2 mt-2">
                       <Link href={`/berita/${berita.slug}`}>{berita.title}</Link>
                     </h3>
                  </div>
                </div>
  ...
  try {
    ...
    return {
      props: {
        ...
        initialBerita: finalBeritaTerbaru,
