  ...
  const fetchStats = async () => {
    try {
      const [
        ...
        supabase.from('berita').select('id, views').eq('status', 'Published'),
        supabase.from('berita').select('id').neq('status', 'Published'),
        supabase.from('videos').select('id'),
        supabase.from('categories').select('id'),
        supabase.from('ads').select('id').eq('is_active', true),
        supabase.from('berita').select('id, title, author, category, status, views').order('views', { ascending: false }).limit(5),
      ]);
    ...
    <AdminLayout>
      ...
      {loading ? (
        <div className="text-center py-20 text-gray-400">
          <i className="fa-solid fa-circle-notch animate-spin mr-2"></i>Memuat statistik...
        </div>
      ) : (
        <>
          {/* Stats Cards */}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
            <div className="px-6 py-5 border-b border-gray-200 flex items-center justify-between">
              ...
              <Link href="/admin/berita" className="text-base text-blue-700 hover:text-blue-900 font-bold hover:underline">
                Lihat Semua Berita →
              </Link>
            </div>
